...
async function renderToHTML(req, res, pathname, query, renderOpts) {
    ...
    if (renderServerComponentData) {
        ...
        const piper = (push, next)=>{
            bufferedReadFromReadableStream(reader, push).then(()=>next()
   ...
   */ const generateStaticHTML = supportsDynamicHTML !== true;
    ...
    if (concurrentFeatures) {
        ...
        let result = '';
        const readable = ReactDOMServer.renderToReadableStream(document, {
            onError: (e)=>{
                throw e;
            }
        });
...
function renderToWebStream(ReactDOMServer, element, suffix, serverComponentsInlinedTransformStream, generateStaticHTML) {
            ...
            if (!resolved) {
                ...
                resolve((push, next)=>{
                    ...
                    Promise.all([
                        ...
                        inlinedDataReader && bufferedReadFromReadableStream(inlinedDataReader, push), 
                    ]).then(()=>{
                        push([
                            closeTag
                        ]);
...
function chainPipers(pipers) {
    return pipers.reduceRight((lhs, rhs)=>(push, next)=>{
...
function piperFromArray(strings) {
    const encoder = new TextEncoder();
...
function piperToString(input) {
    return new Promise((resolve, reject)=>{
        ...
        let bufferedString = '';
        input((chunks)=>chunks.forEach((chunk)=>bufferedString += textDecoder.decode(chunk)
            )
        , (err)=>{
            if (err) {
                reject(err);
